...
# cursor = db.cursor()
# cursor.execute("SELECT sentiment_score, category, location FROM LSTM_Posts")
...
df = pd.read_csv('Bluesky_Disaster_Predictions_With_Relevance.csv', usecols=['sentiment_score', 'category', 'location'])
...
# location_stats = df.groupby('category').agg(
